...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
   ...
   //--------------------------------------------------
    ...
    double var = estim::EstimChipNoise(src_tx_usrp, pow(2,15),rx_ch_sel_bits, "../../data/fwd_alb_noise_samps.dat"); 
    ...
    int D_test = 0;
    for(int i = 0; i<100;i++) {
    auto ch_params = estim::ChEstim(src_tx_usrp, D_test, rx_ch_sel_bits, tx_core_bits, gpio_start_sel_bits, pow(2,12), "../../data/fwd_alb_prmbl_samps.dat");
    ...
    int D_eff = D_hat;
    estim::CompensateDelays(src_tx_usrp, D_eff);

    std::complex<double> h_comp = h_hat/std::abs(h_hat); 
    estim::PhaseEq(src_tx_usrp, h_comp);
    ...
    mmio::StartTx(src_tx_usrp, mode_bits, rx_ch_sel_bits, tx_core_bits, gpio_start_sel_bits);
    
    while(true) {
        //Run and check received pkt    
        mmio::WrMmio(src_tx_usrp,0x0,0x0); //need to clear addr buffer, not sure why its 0x8. 0x0 should work fine...
        bool pkt_valid = mmio::RdMmio(src_tx_usrp, mmio::kBbStatusAddr) & 0x2; //around 10 ms
        if(pkt_valid)
            break;
    }
    ...
    for(int i = 0; i*32 < mmio::kPktLen; i++) {
        output_pkt[i] = mmio::RdMmio(src_tx_usrp, mmio::kOutPktAddr+i);
    ...
    mmio::ReadSampleMem(src_tx_usrp, 1, std::pow(2,16), "../../data/fwd_alb_samps.dat"); 
    
    mmio::ReadSampleMem(src_tx_usrp, 0, std::pow(2,16), "../../data/fb_alb_samps.dat"); 
